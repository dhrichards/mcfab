...
def Normalisation(Ecc,Eca,D):

    I = jnp.eye(3)

    a2 = I/3
    a4 = (1/15)*(jnp.einsum('ij,kl->ijkl',I,I) \
            + jnp.einsum('ik,jl->ijkl',I,I) \
            + jnp.einsum('il,jk->ijkl',I,I))

    F = Fluidity(Ecc,Eca,1.0,a2,a4)
    S = jnp.linalg.tensorsolve(F,D)

    A = jnp.sqrt(jnp.einsum('ij,ji',S,S)/jnp.einsum('ij,ji',D,D))
    return A
...
def int_inveta_ck(s,Ecc,Eca,c,m,k):
    #For n=3
    ak = akcalc(c,m,k)
    akp2 = akcalc(c,m,k+2)
    akp4 = akcalc(c,m,k+4)
...
def Rathmann1A(D,Ecc,Eca,a2,a4):
...
def Rathmann3(D,Ecc,Eca,c,m):
    ...
    def objective(x,D,Ecc,Eca,c,m):
        ...
        nc0 = int_inveta_ck(S,Ecc,Eca,c,m,0)
        nc2 = int_inveta_ck(S,Ecc,Eca,c,m,2)
        nc4 = int_inveta_ck(S,Ecc,Eca,c,m,4)
...
# def Normalisation(Ecc,Eca,power):
#     # Factor to normalise the tensor by, based on isotropic fluidity
    
#     # Ideal isotropic tensors
#     I = jnp.eye(3)
#     a2 = I/3
#     a4 = (1/15)*(jnp.einsum('ij,kl->ijkl',I,I) \
#             + jnp.einsum('ik,jl->ijkl',I,I) \
